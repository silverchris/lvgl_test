...
typedef struct {
    ...
    esp_lcd_panel_handle_t panel; //RGB panel
                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    // Create RGB panel
    ESP_GOTO_ON_ERROR(esp_lcd_new_rgb_panel(vendor_config->rgb_config, &st7701->panel), err, TAG, "create RGB panel failed");
...
static esp_err_t panel_st7701_init(esp_lcd_panel_t *panel)
{
    ...
    // Init RGB panel
    ESP_RETURN_ON_ERROR(st7701->panel->init(st7701->panel), TAG, "init RGB panel failed");
...
static esp_err_t panel_st7701_reset(esp_lcd_panel_t *panel)
{
    ...
    // Reset RGB panel
    ESP_RETURN_ON_ERROR(st7701->panel->reset(st7701->panel), TAG, "reset RGB panel failed");
...
static esp_err_t panel_st7701_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    if (st7701->flags.mirror_by_cmd) {
        ...
        ESP_RETURN_ON_ERROR(st7701->panel->mirror(st7701->panel, mirror_x, mirror_y), TAG, "RGB panel mirror failed");
    }
    return ESP_OK;
}
...
static esp_err_t panel_st7701_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    if (st7701->flags.display_on_off_use_cmd) {
        ...
        ESP_RETURN_ON_ERROR(st7701->panel->disp_on_off(st7701->panel, on_off), TAG, "RGB panel disp_on_off failed");
    }
    return ESP_OK;
}
